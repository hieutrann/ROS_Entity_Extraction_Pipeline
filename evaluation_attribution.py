...
# parser.add_argument("--filename", type=str, default='sample_226')
...
for sample in list_file:
    ...
    with open(f"{model_folder}/{sample}/output_extract.json",'r') as file:
    ...
    with open(f"{model_folder}/{sample}/output_classify.json",'r') as file:
        pred_converted = json.load(file)
    ...
    import csv
    with open("sim_metric.csv", "a", newline="") as f:
        writer = csv.writer(f)
